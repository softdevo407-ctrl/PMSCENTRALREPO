...
export interface AuthContextType {
  ...
  loginWithCAS: (employeeCode: string) => Promise<boolean>;
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const loginWithCAS = useCallback(async (employeeCode: string): Promise<boolean> => {
    setIsLoading(true);
    setError(null);
    try {
      const response = await authService.loginWithCAS(employeeCode);
      ...
      if (response.success) {
        ...
        setError(response.message || 'CAS Login failed');
        return false;
      }
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : 'CAS Login failed';
      setError(errorMsg);
      return false;
    } finally {
      setIsLoading(false);
